...
def parse_int(details):
    ...
    if (
            is_char
            and size is None
            and get_detail(details, "note", "idl", "type") == "wchar"
    ):
        size = 16
    elif is_bool:
        size = 8
...
def parse_string(details):
    return PrimitiveType(
        PrimitiveTypeTraits(
            element_size=16
            if get_detail(details, "note", "idl", "type") == "wstring"
            else 8,
            is_text=True,
            is_scalar=False,
        )
    ...
    for field_dict in details["fields"]:
        if "sequence" in field_dict["type"]:
            sequence = parse_sequence(
                types, {"type": field_dict["type"], "capacity": 1, "size": None}
            )
            sequence.set_name(itl_name=sequence.base_type.name.itl_name)
            struct_type.add_field(
                field_dict["name"], sequence, field_dict.get("optional", False)
            )
        else:
            struct_type.add_field(
                field_dict["name"],
                parse_type(types, field_dict["type"]),
...
def parse_type(types, details):
    ...
    if details_type is str:
        ...
        raise TypeError(
            'Type "{}" is not a valid type for parse_type()'.format(
                details_type.__name__
            )
        )
