...
def main():
    ...
    argparser.add_argument(
        ...
        metavar="IDL_LIBRARY_CMAKE_NAME",
        type=str,
...
and the name of the Python package.""",
    ...
    argparser.add_argument(
        "--idl-library-build-dir",
        metavar="IDL_LIBRARY_BUILD_DIR",
        type=Path,
        help="""\
...
the exepcted config file is not there, then this option becomes required.""",
    ...
    argparser.add_argument(
        "--dump-ast", action="store_true", help="Print the AST before processing it."
    )
    argparser.add_argument(
        "--just-dump-ast", action="store_true", help="Just print the AST."
    )
    ...
    if not config_path.is_file():
      ...
      directory for it (where cmake was ran) using --idl-library-build-dir""".format(
                config_path
            )
        )
